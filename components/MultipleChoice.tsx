...
const MultipleChoice: React.FC<MultipleChoiceProps> = ({ onBack }) => {
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    setError(null);
    try {
      const newQuestions = await generateMCQs(10);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center text-center">
        <Spinner />
        <p className="mt-4 text-lg">Génération du quiz en cours...</p>
      </div>
    );
  }
  ...
  if (isFinished) {
    return (
      <div className="text-center bg-white dark:bg-slate-800 p-8 rounded-lg shadow-xl w-full max-w-lg">
        ...
        <p className="text-xl mb-6">Votre score : <span className="font-bold text-blue-500">{score} / {questions.length}</span></p>
        <div className="flex justify-center space-x-4">
          <button onClick={fetchQuestions} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors">Nouveau Quiz</button>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-lg shadow-xl">
        ...
        {selectedAnswer && (
          <div className="mt-6 text-center">
            ...
            <button
              onClick={handleNext}
              className="mt-4 bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700 transition-colors w-full sm:w-auto"
            >
